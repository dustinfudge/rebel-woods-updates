...
import { formatPhoneNumber } from "@/lib/phoneNumbers";
...
const danger = "inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-[#d7a18b] bg-[#fff7f2] px-4 py-2 text-sm font-bold text-[#8b3e22] disabled:opacity-50";
...
export function AdminSetupWorkspace(): React.JSX.Element {
  ...
  const archivedHorses = horseViews.filter((horse) => !horse.is_active);
  ...
  async function addHorse(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    await mutate(async () => {
      ...
      const { data: horse, error } = await client.from("horses").insert({
        ...
        name: value(formData, "horseName"), field_id: null,
        herd_id: null, ...horseInformationFrom(formData),
      }).select("*").single();
      if (error) throw error;
  ...
  async function archiveHorse(horse: Horse): Promise<void> {
    await mutate(async () => {
      const { error } = await getSupabaseBrowserClient().from("horses").update({ is_active: false }).eq("id", horse.id);
      if (error) throw error;
      setSelectedHorseId(null);
      await refresh(`${horse.name} was removed from the active stable. Their records were preserved.`);
    });
  }

  async function restoreHorse(horse: Horse): Promise<void> {
    await mutate(async () => {
      const { error } = await getSupabaseBrowserClient().from("horses").update({ is_active: true }).eq("id", horse.id);
      if (error) throw error;
      await refresh(`${horse.name} was restored to the active stable.`);
    });
  }
  ...
  async function setPersonActive(person: Profile, isActive: boolean): Promise<void> {
    if (!profile) return;
    await mutate(async () => {
      if (person.id === profile.id && !isActive) throw new Error("You cannot deactivate your own administrator account.");
  ...
  async function deleteHealthRecord(record: HealthRecord): Promise<void> {
    const recordDate = formatHealthRecordDate(record.recorded_on);
    if (!window.confirm(`Delete ${record.item_name} from ${recordDate}? This cannot be undone.`)) return;
    await mutate(async () => {
      const { error } = await getSupabaseBrowserClient().from("horse_health_records").delete().eq("id", record.id);
      if (error) throw error;
      await refresh(`${record.item_name} was deleted from the health history.`);
    });
  }
  ...
  async function updateRetention(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    await mutate(async () => {
      ...
      if (error) throw error;
      await refresh(`Conversation messages and media will be kept for ${retentionDays} days.`);
    });
  }
...
function Locations({ fields, herds, retentionDays, saving, onAdd, onUpdateRetention }: { readonly fields: readonly Field[]; readonly herds: readonly Herd[]; readonly retentionDays: number; readonly saving: boolean; readonly onAdd: (event: FormEvent<HTMLFormElement>, kind: "field" | "herd") => Promise<void>; readonly onUpdateRetention: (event: FormEvent<HTMLFormElement>) => Promise<void> }): React.JSX.Element {
  return <section><Intro step="Step three" title="Where do the horses live?">Add the field and herd names your team already uses. You can add more later.</Intro><div className="grid gap-5 md:grid-cols-2"><LocationCard title="Fields" example="North Field, Creek Field" items={fields} saving={saving} onSubmit={(event) => onAdd(event, "field")} /><LocationCard title="Herds" example="Willow Herd, Oak Herd" items={herds} saving={saving} onSubmit={(event) => onAdd(event, "herd")} /></div><div className="mt-5"><Card title="Conversation storage" description="Choose how long messages, photos, and videos remain available in each horse’s conversation."><form className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end" onSubmit={(event) => void onUpdateRetention(event)}><Label name="Keep conversation history for"><select className={input} defaultValue={retentionDays} key={retentionDays} name="retentionDays"><option value="90">90 days</option><option value="180">180 days (recommended)</option><option value="365">1 year</option><option value="730">2 years</option></select></Label><button className={primary} disabled={saving} type="submit"><CalendarClock size={17} />Save retention</button></form><p className="mb-0 mt-4 rounded-xl bg-[#e4ece4] p-4 text-sm leading-6 text-[#385943]">When cleanup is enabled, it will run weekly. Horse information, care cards, medication history, people, access, and the date of the last staff communication are always preserved.</p></Card></div></section>;
}
...
function AddHorseCard({ saving, onAdd }: { readonly saving: boolean; readonly onAdd: (event: FormEvent<HTMLFormElement>) => Promise<void> }): React.JSX.Element {
  return <Card title="Add a horse" description="New horses begin unassigned so they can be placed with the visual herd manager."><form className="space-y-4" onSubmit={(event) => void onAdd(event)}><Label name="Horse name"><input className={input} maxLength={100} name="horseName" required /></Label><HorseInformationFields /><CareFields /><button className={primary} disabled={saving} type="submit"><Plus size={17} />Add horse</button></form></Card>;
}
...
function HealthRecordsCard({ horse, saving, onAdd, onDelete }: { readonly horse: HorseView; readonly saving: boolean; readonly onAdd: (event: FormEvent<HTMLFormElement>, options: readonly HealthRecordOption[]) => Promise<void>; readonly onDelete: (record: HealthRecord) => Promise<void> }): React.JSX.Element {
  ...
  return <Card title="Vaccines, Tests & Documents" description="Add the date given or completed. The newest date for each item appears first; full history is preserved.">
    <div className="mb-5 grid gap-4 sm:grid-cols-2">
      <HealthRecordLatestGroup title="Latest Vaccines" records={vaccinationRecords} saving={saving} onDelete={onDelete} />
      <HealthRecordLatestGroup title="Latest Tests & Documents" records={testRecords} saving={saving} onDelete={onDelete} />
    </div>
    {horse.vaccine_schedule.trim() ? <div className="mb-5 rounded-xl border border-[#dedfd8] bg-[#f7f3e9] p-4 text-sm"><strong className="block text-[#385943]">Pre-Existing Conditions</strong><p className="mb-0 mt-1 whitespace-pre-wrap text-[#68736b]">{horse.vaccine_schedule}</p></div> : null}
    ...
    {horse.healthRecords.length > 0 ? <details className="mt-4 rounded-xl border border-[#dedfd8] bg-white p-3"><summary className="cursor-pointer text-sm font-bold text-[#385943]">Full Health History · {horse.healthRecords.length}</summary><div className="mt-3 space-y-2">{[...horse.healthRecords].sort((left, right) => right.recorded_on.localeCompare(left.recorded_on)).map((record) => <HealthRecordRow key={record.id} record={record} saving={saving} onDelete={onDelete} />)}</div></details> : null}
  </Card>;
}

function HealthRecordLatestGroup({ records, saving, title, onDelete }: { readonly records: readonly HealthRecord[]; readonly saving: boolean; readonly title: string; readonly onDelete: (record: HealthRecord) => Promise<void> }): React.JSX.Element {
  return <section className="rounded-2xl bg-[#e4ece4] p-4"><h4 className="mb-3 font-bold text-[#1d3528]">{title}</h4>{records.length === 0 ? <p className="mb-0 text-sm text-[#68736b]">Nothing entered yet.</p> : <div className="space-y-2">{records.map((record) => <HealthRecordRow key={record.id} record={record} saving={saving} onDelete={onDelete} />)}</div>}</section>;
}

function HealthRecordRow({ record, saving, onDelete }: { readonly record: HealthRecord; readonly saving: boolean; readonly onDelete: (record: HealthRecord) => Promise<void> }): React.JSX.Element {
  return <div className="rounded-xl bg-white p-3 text-sm"><div className="flex flex-wrap items-start justify-between gap-2"><strong>{record.item_name}</strong><div className="flex items-center gap-2"><time className="font-bold text-[#385943]" dateTime={record.recorded_on}>{formatHealthRecordDate(record.recorded_on)}</time><button aria-label={`Delete ${record.item_name} from ${formatHealthRecordDate(record.recorded_on)}`} className="inline-flex min-h-8 items-center gap-1 rounded-full border border-[#d7a18b] bg-[#fff7f2] px-2.5 text-xs font-bold text-[#8b3e22]" disabled={saving} onClick={() => void onDelete(record)} type="button"><Trash2 size={13} />Delete</button></div></div>{record.result_notes.trim() ? <p className="mb-0 mt-1 whitespace-pre-wrap text-[#68736b]">{record.result_notes}</p> : null}</div>;
}
...
function People({ access, currentProfileId, horses, owners, profiles, saving, onContinue, onDelete, onGrant, onInvite, onRemoveAccess, onSetActive, onUpdatePhone }: PeopleProps): React.JSX.Element {
  ...
  const inactiveProfiles = profiles.filter((person) => !person.is_active);

  return <section>
    <Intro step="Step two" title="Invite owners and connect them to horses.">Owners and authorized family receive the same horse access. Their phone numbers appear on each connected horse’s information card.</Intro>
