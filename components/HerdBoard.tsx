...
export function HerdBoard({ herds, horses, onBack, onMoveHorse }: HerdBoardProps): React.JSX.Element {
  ...
  const columns = useMemo<readonly HerdColumn[]>(() => [
    { herdId: null, title: "Not in a herd" },
    ...herds.map((herd, index) => ({ herdId: herd.id, title: `Herd ${index + 1}` })),
  ], [herds]);
  ...
  return <section>
    ...
    <div className="mb-5 rounded-[2rem] bg-[#1d3528] p-6 text-white shadow-xl sm:p-8">
      ...
      <p className="mb-0 max-w-3xl leading-7 text-[#cdd9cf]">Drag a horse into another box. On a phone, tap a horse and then tap <strong>Move here</strong> in the destination herd.</p>
    </div>
    ...
    <div className="flex snap-x gap-4 overflow-x-auto pb-5" aria-label="Herd groups">
      {columns.map((column) => {
        ...
        >
          <header className="mb-4 flex min-h-12 items-center justify-between gap-3 border-b border-[#dedfd8] pb-3">
            <span><strong className="block font-serif text-2xl text-[#1d3528]">{column.title}</strong><small className="font-bold text-[#68736b]">{columnHorses.length} {columnHorses.length === 1 ? "horse" : "horses"}</small></span>
